...
function getApiUrl() {
  ...
  // Always clear any stale local IP from localStorage
  const storedUrl = localStorage.getItem('diary_api_url');
  if (storedUrl) {
    const isLocalIP = /localhost|127\.0\.0\.1|172\.|192\.168\.|10\./.test(storedUrl);
    if (isLocalIP) {
      localStorage.removeItem('diary_api_url');
    } else if (storedUrl.trim()) {
      return formatApiEndpoint(storedUrl);
    }
  }

  // On Capacitor (native Android/iOS) or file:// protocol — always use live server
  if (typeof window.Capacitor !== 'undefined' || window.location.protocol === 'file:') {
    return DEFAULT_RENDER_API;
  }

  // On real localhost dev server (with a port)
  if ((window.location.hostname === 'localhost' || window.location.hostname === '127.0.0.1') && window.location.port) {
    return `${window.location.protocol}//${window.location.hostname}:3000/api`;
  }

  // Default: live Render backend
  return DEFAULT_RENDER_API;
}
const API = getApiUrl();
